function Objects= HaarCasadeObjectDetection(IntegralImages,HaarCasade,Options)
    ...
    ScaleWidth = IntegralImages.width/(HaarCasade.width*1.5);
    ScaleHeight = IntegralImages.height/(HaarCasade.height*1.5);
...
%     tic
    ...
    for i=1:itt
        ...
        w = floor(HaarCasade.width*Scale);
        h = floor(HaarCasade.height*Scale);
        ...
        % Stop when enough faces have been found
        if length(x) > Options.LimitClusters
             break
        end
