function [data_out, basisvectors] = kernelpca_tutorial(data_in,num_dim)
%
...
eigvec = eigvec(:,index);
basisvectors = zeros(size(eigvec,1), num_dim);

%% Projecting the data in lower dimensions
data_out = zeros(size(data_in,2), num_dim);
for count = 1:num_dim
    data_out(:,count) = eigvec(:,count)'*K_center';
    basisvectors(:, count) = eigvec(:,count);
end